...
日报的 Markdown、JSON、飞书卡片和仪表盘会直接回答“天板能不能追、地板能不能抄”，并展示确认后次日开盘入场的样本外胜率、3/5日收益、5%尾部和路径回撤。当日首板、炸板、天地板、跌停打开、封跌停和地天板只作为结论后的事件明细；当前全市场可执行回测期望均为负，因此追板与抄底通道默认关闭，不会自动创建交易计划。
...
涨跌停通道使用独立事件回测，不与上述波段策略混算。除“触板价理想成交”基线与收盘形态后验研究外，还单独输出可执行口径：收盘确认后，下一交易日开盘未封涨停且有成交时才入场，收益从该开盘价起算；后续封死跌停或停牌时退出顺延。涨停侧覆盖首板、主线首板和高位连板；跌停侧覆盖跌停打开、主线跌停打开和封跌停：
...
- 主板普通股按10%；可识别的沪深主板ST股在2026-07-06前按5%、之后按10%；科创板和注册制创业板按20%、北交所按30%识别涨跌停。历史名称无法完整还原时，ST涨跌停识别属于已知数据限制。
...
`unmapped_pullback.candidates` 专门承接主题篮子之外的相对强度（RS，relative strength，相对强弱）名字，并把其中具备回踩结构的标的升级为条件化交易计划：

- 复用未映射强势的横截面分位、5/20 日涨幅、量能和靠近高点约束，再检查约 60 日窗口内“先推进 ≥15%，再自高点回撤 ≥6%”的 pullback/reclaim（回踩再站稳）结构。
...
- `shadow_account`：独立 10 万人民币影子账户单例，记录现金、市值和净值。每个行情日的现金、持仓、事件和净值由 `apply_shadow_day` 一次事务写入；`as_of` 只在提交成功后推进。
